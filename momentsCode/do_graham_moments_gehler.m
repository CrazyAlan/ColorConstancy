...
%{
   \ednote{OMIT FROM FINAL: Proof of eq.(\ref{EQ:MOMENTS2}):\\
   $$
    1/2 \partial/\partial \alpha_i \left \{
    (\alpha_i M_i \mbC - W_i)(\alpha_i \mbC^T M_i^T - W_i^T)
   $$
   $$
    \Rightarrow \; \alpha_i (M_i \mbC) (M_i \mbC)^T \;=\; W_i (M_i \mbC)^T
    \\
    \Rightarrow \; \alpha_i  \;=\; W_i (M_i \mbC)^+
   $$
   }
%}
    end % for k
