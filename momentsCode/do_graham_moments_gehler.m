...
%    readgehler;
load '../grayBall/real_illum_11346.mat' grayBallIllum % allcanon5Dsmall = zeros(482,183,275,3); % all portrait
[howmanycands, r,c, n3] = size(allcanon5Dsmall); % 482   183   275 3
% getGehlerLights; % gets alllightschrom

load('../grayBall/real_illum_11346.mat');
alllights=real_rgb; clear real_rgb % 11345 illuminants
...
for k=1:K
    ...
    for i=1:MaxItn
        ...
        for j=1:size(M,1)
            ...
            D(j,j)= W(j,:)*pinv(mc); % same as below:
        end % for j
    end % for i
