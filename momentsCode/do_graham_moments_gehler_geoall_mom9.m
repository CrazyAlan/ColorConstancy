% do_graham_moments_gehler_geoall_mom9.m
...
for i=testindices
    im=squeeze(allim(i,:,:));
        ...
        bright = lum>quantile(lum,0.95);
            %bright=ones(r*c,1);
    %M(i,:) = moments9(im);
    anM =[ moments9_geo(makechrom3vec(im)) ...
        moments9(im)];
    %M(i,:) = moments9_geo(makechrom3vec(im(:,:)));
%M(i,:) = moments9(im);
...
% 3.2659    2.5664    2.6233    0.0479    8.8885 using bright pixels
%3.0632    2.6293    2.6342    0.2059    8.0447 using all pixels
% Recall, with mean() not mygeo_glog, we had:
% 3.2470    2.3237    2.4964    0.0456    8.6559
